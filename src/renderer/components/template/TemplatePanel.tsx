...
export default function TemplatePanel({ projectId, onInsert }: TemplatePanelProps) {
  ...
  const renderTemplateItem = (tmpl: PromptTemplate) => {
    ...
    return (
      <div
        ...
        className={`flex items-start justify-between gap-2 px-2 py-1.5 rounded hover:bg-elevated/50 group ${
          onInsert ? 'cursor-pointer' : ''
        }`}
      >
        <div className="flex-1 min-w-0">
          <p className="text-xs text-fg truncate">{tmpl.title}</p>
          <p className="text-[10px] text-fg-dim truncate">{firstLine}</p>
        </div>
        <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button
            onClick={() => setEditing(tmpl.id)}
            className="px-1.5 py-0.5 text-[10px] text-fg-muted hover:text-fg hover:bg-hover rounded transition-colors"
          >
            {t('common.edit')}
          </button>
          <button
            onClick={() => handleDelete(tmpl.id)}
            className="px-1.5 py-0.5 text-[10px] text-fg-muted hover:text-red-400 hover:bg-hover rounded transition-colors"
          >
            {t('template.del')}
          </button>
        </div>
  ...
  const renderSection = (label: string, items: PromptTemplate[]) => (
    <div>
      <h3 className="text-[10px] font-medium text-fg-dim uppercase tracking-wider px-2 mb-1">
        {label}
      </h3>
      {items.length === 0 ? (
        <p className="text-[10px] text-fg-dimmer px-2 py-2">{t('template.noTemplates')}</p>
      ) : (
