...
export default function PaneStatusBar({ panes, minimizedPanes, onRestore }: PaneStatusBarProps) {
  ...
  return (
    <div className="h-[22px] bg-base border-t border-border-subtle flex items-center gap-px px-1 shrink-0">
      {minimizedPanes.map((mp) => {
        ...
        return (
          <button
            ...
            className="h-[18px] px-1.5 text-[11px] leading-none text-fg-dim
                       border-r border-border-default
                       hover:text-fg-secondary hover:bg-hover/60 transition-colors"
          >
            {hasClaudeActivity && (
